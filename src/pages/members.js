export const members = [
    {
      name: "Tsuyoshi🐢",
      photo: "/tsuyoshi.jpg",
      personalbest: {
        "100m": "12.03",
        "400m": "53.00",
        "800m": "1:57.17",
        "1500m": "4:05.07",
        "5000m": "15:50.??",
        "Marathon": "2:32:43",
        "Long jump": "5m40cm"
      },
      intro: "弊チーム代表。シューズをいっぱい持っている。普段は研究者。ランニングも探求らしい。"
    },
    {
      name: "Moeko🐹",
      photo: "/moeko.png", // 画像パス
      personalbest: {
        "1500m":"4:45.65",
        "3000m": "9:46.06",
        "5000m": "16:35.99",
        "Marathon": "3:21:25"
      },
      intro: "弊チームの紅一点。中学時にはジュニアオリンピック、高校時は都道府県対抗駅伝の北海道代表、空知記録保持者と輝かしい経歴。普段はエンジニア。本ホームページも担当。重い腰を上げて高校ぶりの自己記録更新で空知記録更新なるか！？"
      },
      {
        name: "Sho🐶",
        photo: "/blog-placeholder-2.jpg", // 画像パス
        personalbest: {
          "400m": "49.99",
          "Marathon": "3:33"
        },
        intro: "弊チームのロングスプリンター。800mアプローチで400mも走る。マラソンも走る。"
        },
        {
            name: "Takuto🐰",
            photo: "/blog-placeholder-4.jpg", // 画像パス
            personalbest: {
              "100m": "11.32",
              "Enbannage": "5m"
            },
            intro: "弊チームのスプリンター。理学療法士をもっているエンジニア。"
            },
         {
            name: "Hiroki",
            photo: "/blog-placeholder-4.jpg", // 画像パス
            personalbest: {
              "Marathon": "2:36",
            },
            intro: "陸上未経験、社会人から走り始めてマラソン2:36を達成したスーパーランナー。"
            },
         {
            name: "Koya",
            photo: "/blog-placeholder-4.jpg", // 画像パス
            personalbest: {
              "100m": "11.10"
            },
            intro: "我がチームのチーター。陸上でしかドーパミンが出ないモンスター。世界陸上は録画しない、Day9まで現地観戦するのが彼の流儀。"
            },

  ];